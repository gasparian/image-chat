...
from agent.structures import (
    DetectionResult, ChatAgentOutput, ChatMessage, CaptionData, Provenance
)
...
3. Object detection results (if available)
4. Previous conversation history (if available)
...
def answer_question_node(state: ChatAgentState) -> ChatAgentState:
    with track_step("answer_question"):
        ...
        previous_detections = state.get("previous_detections")
        previous_class_vocab = state.get("previous_class_vocab")
        ...
        if previous_detections and previous_class_vocab:
            detection_summary = f"\nPrevious detection results:"
            detection_summary += f"\n- Total objects detected: {len(previous_detections)}"
            detection_summary += f"\n- Object classes found: {', '.join(previous_class_vocab)}"
            ...
            context_parts.append(detection_summary)
            logger.info("Using previous detection results", object_count=len(previous_detections))
        ...
        if previous_detections:
            steps.append("using_previous_detections")
