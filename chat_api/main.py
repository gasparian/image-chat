...
from agent.graph.nodes.intent_routing import route_intent, IntentType
from agent.graph.chat_graph import run_chat_graph
from agent.visualize import visualize_and_save
...
async def stream_chat_response(session_id: str, message: str) -> AsyncGenerator[str, None]:
    ...
    try:
        ...
        if intent == IntentType.DETECTION:
            yield f"event: status\ndata: {json.dumps({'message': 'Running object detection...'})}\n\n"
            await asyncio.sleep(0.05)
            ...
            class_vocab = result.class_vocab or []

            viz_path = None
            if detections and session.image_path:
                viz_path = visualize_and_save(
                    session.image_path,
                    result
                )

            response_text = f"Found {len(detections)} objects: {', '.join(class_vocab)}"
            ...
            yield f"event: status\ndata: {json.dumps({'message': 'Generating caption...'})}\n\n"
            await asyncio.sleep(0.05)
            ...
            # TODO: To get real token-level streaming, refactor to call LLM directly with stream=True
